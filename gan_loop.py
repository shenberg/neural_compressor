...
from models import Generator, Encoder, AutoEncoder
from utils import DatasetSubset, mix_samples, _mix_samples, load_batches, batchify
import orthoreg
...
params = dict(
    MODE = 'wgan-loop', # Valid options are dcgan, wgan, or wgan-gp
...
#    GENERATOR_GP = GENERATOR_GP,
    ...
    DISCRIMINATOR_L2_LAMBDA = DISCRIMINATOR_L2_LAMBDA,
    NET_D = NET_D,
...
netD = Encoder(CRITIC_DIM, INPUT_DIM, use_layer_norm=ENCODER_LAYER_NORM)
...
if USE_TENSOR_DATASET:
    ...
    train_dataset = batchify(torchvision.datasets.ImageFolder(DATA_DIR, transform=transform))
...
for iteration in tqdm(range(ITERS)):
    ...
    for i in range(next(CRITIC_GEN)):
        ...
        l2_loss = DISCRIMINATOR_L2_LAMBDA * mse_loss(fake, real_target_v)
        l2_loss += DISCRIMINATOR_L2_LAMBDA * mse_loss(netG(fake_enc), autograd.Variable(fake.data,volatile=True))
        
        errD = torch.sqrt(mmd2) + lambda_rg * one_side_errD - l2_loss
    ...
    Wasserstein_D = -(loss.data - gradient_penalty.data * LAMBDA - ortho_loss_v.data - l2_loss.data)
    D_cost = loss.data
    ############################
    # (2) Update G network
    ###########################
