...
class Generator(nn.Module):
    def __init__(self):
        ...
        block1 = nn.Sequential(
            ...
            nn.ConvTranspose2d(4 * DIM, 2 * DIM, KERNEL_SIZE, stride=2, padding=1, bias=False),
            nn.ReLU(True),
        )
        block2 = nn.Sequential(
            nn.InstanceNorm2d(2 * DIM, affine=True),
            nn.ConvTranspose2d(2 * DIM, DIM, KERNEL_SIZE, stride=2, padding=1, bias=False),
            nn.ReLU(True),
        )
        ...
        self.out_norm = nn.InstanceNorm2d(3)
...
class Discriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        before_linear = output.view(-1, 4 * 4 * 4 * DIM)
...
def calc_gradient_penalty(netD, netG, real_data, fake_data, real_data_grad):
...
for iteration in range(ITERS):
    ...
    for i in range(next(CRITIC_ITERS)):
        _data = next(gen)
        netD.zero_grad()
        ...
        real_data_v = autograd.Variable(real_data, requires_grad=True)
        ...
        gradient_penalty = calc_gradient_penalty(netD, netG, real_data_v.data, fake.data, real_data_v.grad)
